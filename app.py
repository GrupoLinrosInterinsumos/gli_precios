...
def get_val(man, ex, default):
    try:
        if man is not None and not pd.isna(man): return float(man)
        if ex is not None and not pd.isna(ex): return float(ex)
...
def get_core_name(name):
    core = re.sub(r'\bX?\s*\d+(?:[\.,]\d+)?\s*(?:KG|KGS|KILO|KILOS|G|GR|GRS|L|LT|LTS|LITRO|LITROS|ML|LB|LBS|GAL|GALON|GALONES)\b', '', str(name), flags=re.IGNORECASE)
    core = re.sub(r'[^a-zA-Z0-9\s]', '', core)
    return re.sub(r'\s+', ' ', core).strip()

def get_quantity_normalized(name):
    match = re.search(r'\bX?\s*(\d+(?:[\.,]\d+)?)\s*(KG|KGS|KILO|KILOS|G|GR|GRS|L|LT|LTS|LITRO|LITROS|ML|LB|LBS|GAL|GALON|GALONES)\b', str(name), flags=re.IGNORECASE)
    if match:
        try: 
...
def es_excepcion_herencia(nombre):
    n_clean = re.sub(r'\s+', '', str(nombre)).upper()
...
def subir_maestro():
    try:
        ...
        for _, row in df.iterrows():
            ...
            if es_nativo_soles(nombre): pass 
            elif es_proveedor_soles_mixto(nombre, prov): c_fab /= 4.0 
            elif es_excepcion_soles_clasica(nombre):
            ...
            p.costo_base_man = None; p.costo_fab_man = None; p.coyuntural_man = None
            p.margen_man = None; p.dscto_pv_man = None; p.dscto_dist_man = None
            ...
            p.pv_str = '' if pv_val.lower() == 'nan' else pv_val[:10]
            p.dist_str = '' if dist_val.lower() == 'nan' else dist_val[:10]
...
def crear_producto():
    try:
        ...
        p = Producto.query.filter_by(nombre=nombre_original).first() if nombre_original else Producto.query.filter_by(nombre=nombre).first()
        
        if p:
            p.nombre = nombre; p.codigo = codigo_val; p.empresa = empresa_val
            ...
            p.pv_str = pv_str_val; p.dist_str = dist_str_val; p.tipo_origen = tipo_origen_val
            p.oculto = False; p.es_manual = True
        else:
    ...
    try:
        ...
        for c in prods:
            if c.tipo_origen == 'COMPRADO' and not c.oculto:
                parent_core = get_core_name(c.nombre)
                p_coyun = get_val(c.coyuntural_man, c.coyuntural_ex, 0.0)
                p_base = get_val(c.costo_base_man, c.costo_base_ex, 0.0)
                data_comprados.append({
                    'nombre': c.nombre, 'costo_usd': p_base, 'coyun_usd': p_coyun, 
                    'core': parent_core, 'clean': c.nombre.replace(' ', '').upper()
                })
    ...
    try:
        ...
        for c in Producto.query.all():
            if c.tipo_origen == 'COMPRADO' and not c.oculto:
                data_comprados.append({
                    'nombre': c.nombre, 
                    'costo_usd': get_val(c.costo_base_man, c.costo_base_ex, 0.0), 
                    'coyun_usd': get_val(c.coyuntural_man, c.coyuntural_ex, 0.0),
                    'core': get_core_name(c.nombre), 'clean': c.nombre.replace(' ', '').upper()
                })
        ...
        for p in prods:
            ...
            if p.tipo_origen == 'FABRICADO' and not es_excepcion_herencia(p.nombre):
                ...
                if c_heredado_usd > 0: 
                    c_base_db = p.costo_base_man if (p.costo_base_man is not None and p.costo_base_man > 0) else c_heredado_usd
                if coyun_heredado_usd > 0:
                    coyun_db = p.coyuntural_man if (p.coyuntural_man is not None and p.coyuntural_man <= 0) else coyun_heredado_usd
            ...
            data.append({
                ...
                "Precio LIMA": round(float(p_lima_final), 2), "Precio PROVINCIA": round(float(p_prov_final), 2), 
                "Visible Ventas": "SÍ" if (p.visible_ventas if p.visible_ventas is not None else True) else "NO", "Nota": str(p.nota)
            })
        ...
        print(f"Error fatal en exportar: {e}")
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
